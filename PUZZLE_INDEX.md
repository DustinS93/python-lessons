...
| Functions — parameter vs argument, a function is an object (parens invoke it), local scope + `NameError`, shadowing a global, no `return` → returns `None` (`None + None` TypeError), `return` exits the whole function so it goes AFTER the loop, string accumulator (`""` + concat), nested calls | `helper.py` |
| CAPSTONE — a function that builds and **returns** a dict, single responsibility (one function per job), a function calling another function, `[::-1]` in a report, `print()` around a `None`-returning function, accumulator created inside the function not outside | `word_stats.py` |
